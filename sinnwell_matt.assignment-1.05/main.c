...
void movePC(Character* pc, map_t * w, Character * characterTable[21][80], int hikerDistance[21][80], int rivalDistance[21][80], int xChange, int yChange, Character** characterQ) {
  ...
  //but don't move
  if (characterTable[moveX][moveY] && characterTable[moveX][moveY]->isBattled == 0) {
...
int main(int argc, char *argv[]){
  ...
  createDistanceMaps(w[xCoord][yCoord], hikerDistance, rivalDistance, characterTable);
  ...
  io_display(w[200][200]->t, characterTable);
    ...
    if (peeking->charValue == '@') {
      ...
      input = getch();
       ...
       if (input == '8'  || input == 'k') {
          ...
          if (validMove(peeking->xCoord, peeking->yCoord, w[200][200], -1, 0)) {
            //if it is not an NPC
            // if (characterTable[peeking->xCoord - 1][peeking->yCoord] == 0) {
              //move
              movePC(peeking, w[200][200], characterTable, hikerDistance, rivalDistance, -1, 0, &characterQ);
            // } 
          } else {
          ...
          //up and left
                  ...
                  // check to see if we are in bounds and not hitting % or ^
          if (validMove(peeking->xCoord, peeking->yCoord, w[200][200], -1, -1)) {
            //if it is not an NPC
            // if (characterTable[peeking->xCoord - 1][peeking->yCoord] == 0) {
              //move
              movePC(peeking, w[200][200], characterTable, hikerDistance, rivalDistance, -1, -1, &characterQ);
            // } 
          } else {
            //can't move that direction
            //print message
          ...
          //right
                  ...
                  // check to see if we are in bounds and not hitting % or ^
          if (validMove(peeking->xCoord, peeking->yCoord, w[200][200], 0, 1)) {
            //if it is not an NPC
            // if (characterTable[peeking->xCoord - 1][peeking->yCoord] == 0) {
              //move
              movePC(peeking, w[200][200], characterTable, hikerDistance, rivalDistance, 0, 1, &characterQ);
            // } 
          } else {
            //can't move that direction
            //print message
          ...
          movePC(peeking, w[200][200], characterTable, hikerDistance, rivalDistance, 0, 0, &characterQ);
       } else if (input == 'Q') {
        continue;
       } else if (input == 't') {
          //open a new window
          ...
          for (int r = 0; r < 21; r++) {
            for (int c = 0; c < 80; c++) {
              if (characterTable[r][c] && characterTable[r][c]->charValue == '@') {
                pcX = r;
                pcY = c;
              }
            }
            ...
            for (int c = 0; c < 80; c++) {
              ...
              if (characterTable[r][c] && characterTable[r][c]->charValue != '@') {
                if (counter <= 20 + downPress && counter >= downPress) {
                  int distX = pcX - characterTable[r][c]->xCoord;
                  int distY = pcY - characterTable[r][c]->yCoord;
                  // char c = characterTable[r][c]->charValue;
                  wprintw(newWindow, "There is a %c with a distance of (%d , %d)\n", characterTable[r][c]->charValue, distX, distY);
          ...
          if (w[200][200]->t[peeking->xCoord][peeking->yCoord] == 4) {
            WINDOW *newWindow = newwin(100, 100, 1, 0);
            refresh();
            ...
            else if (w[200][200]->t[peeking->xCoord][peeking->yCoord] == 5) {
              WINDOW *newWindow = newwin(100, 100, 1, 0);
              refresh();
      ...
      if (characterTable[x][y]->isBattled == 0) {
