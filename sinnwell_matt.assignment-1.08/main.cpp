...
void movePC(Character* pc, map_t * w, Character * characterTable[21][80], int hikerDistance[21][80], int rivalDistance[21][80], int xChange, int yChange, Character** characterQ) {
  ...
  if (characterTable[moveX][moveY] && characterTable[moveX][moveY]->isBattled == 0 && characterTable[moveX][moveY]->charValue != '@') {
    ...
    //tall grass = : = 1
    if (w->t[moveX][moveY] == 1) {
      WINDOW *pokeWindow = newwin(100, 100, 1, 0);
      refresh();
      wprintw(pokeWindow, "You have entered a Pokemon Battle\n");

      //select a pokemon
      //we jave 1092 pokemon
      //vector<pokemon> pokeMon;
      int randPokemonIdRow = getRandomNumber(1092);
      int randPokemonId = pokeMon[randPokemonIdRow].id;

      std::string randPokemonIdToString = std::to_string(randPokemonId);
      wprintw(pokeWindow, "You have selected Pokemon ID: ");
      wprintw(pokeWindow, randPokemonIdToString.c_str());
      wprintw(pokeWindow, "\n");

      //select level
      int distance = findManhattenDistance();
      int pokeLevel = findOriginalPokeLevel(distance);
      string pokeLevelToString = to_string(pokeLevel);
      wprintw(pokeWindow, "The Level is : ");
      wprintw(pokeWindow, pokeLevelToString.c_str());
      wprintw(pokeWindow, "\n");
      ...
      //find pokemonSpeciesId
      int pokemonSpeciesId = findPokemonSpeciesId(randPokemonId);
      string pokemonSepciesIdToString = to_string(pokemonSpeciesId);
      wprintw(pokeWindow, "The Pokemon Species Id is: ");
      wprintw(pokeWindow, pokemonSepciesIdToString.c_str());
      wprintw(pokeWindow, "\n");
      ...
      wprintw(pokeWindow, pokeGender.c_str());
      wprintw(pokeWindow, "\n");
...
void parseFile() {
    ...
    if (inputFile.good()) {
      ...
      while(getline(inputFile, line)) {
        ...
        if (tn.local_language_id == 9) {
          typeNames.push_back(tn);
        }
        
        line = "";
      }

      // for (type_names entry : typeNames) {
  ...
  // if (filesToParse == "moves") {
    success = 1;
    ifstream inputFile1;
    inputFile1.open("/share/cs327/pokedex/pokedex/data/csv/moves.csv");
    if (!inputFile1.good()) {
  ...
  // }
    ...
    success = 1;
    ifstream inputFile2;
    inputFile2.open("/share/cs327/pokedex/pokedex/data/csv/pokemon_moves.csv");
    if (!inputFile2.good()) {
    ...
    if (inputFile2.good()) {
      ...
      // for (pokemon_moves entry : pokemonMoves) {
  ...
  // if (filesToParse == "pokemon_species") {
    ...
    if (inputFile3.good()) {
      ...
      while(getline(inputFile3, line)) {
        ...
        pokemonSpecies.push_back(ps);
        
        line = "";
      }

      // for (pokemon_species entry : pokemonSpecies) {
  ...
  // if (filesToParse == "experience") {
    success = 1;
    ifstream inputFile4;
    inputFile4.open("/share/cs327/pokedex/pokedex/data/csv/experience.csv");
    if (!inputFile4.good()) {
    ...
    // for (experience entry : Experience) {
  ...
  // if (filesToParse == "pokemon_stats") {
    ...
    // for (pokemon_stats entry : pokemonStats) {
  ...
  // if (filesToParse == "stats") {
    success = 1;
    ifstream inputFile6;
    inputFile6.open("/share/cs327/pokedex/pokedex/data/csv/stats.csv");
    if (!inputFile6.good()) {
  ...
  // if (filesToParse == "pokemon_types") {
    ...
    // for (pokemon_types entry : pokemonTypes) {
